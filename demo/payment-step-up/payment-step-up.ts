...
confirmPaymentBtn.addEventListener("click", () => {
  void runAction("confirm-payment", async () => {
    ...
    if (result.decision === StepUpDecision.EnrollmentRequired) {
      setStatus(
        "Enrollment required by backend policy. Use mandatory enrollment flow or allowed fallback strategy.",
        "pending",
      );
      ...
      setStatus(
        lifecycleState.passkeyEnrolled
          ? "Payment approved by passkey step-up."
          : "Payment approved. Optional: suggest passkey enrollment for next payments.",
        "ok",
      );
    } else {
...
function setStatus(message: string, tone: "ok" | "pending" | "error"): void {
  ...
  status.className =
    tone === "ok" ? "status-ok" : tone === "pending" ? "status-pending" : "status-error";
}
