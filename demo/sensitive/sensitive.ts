import { arrayBufferToBase64Url, base64UrlToArrayBuffer, createFetchBackendAdapter, createPasskeyClient, uint8ArrayToBase64Url } from '../../src/index';
...
async function postDemoJson(path: string, payload: Record<string, unknown>): Promise<unknown> {
    ...
    if (!response.ok) {
        throw new Error(
            typeof responseBody === 'object' && responseBody && 'error' in responseBody ? String((responseBody as { error: unknown }).error) : `Request failed with status ${response.status}`,
        );
    }
